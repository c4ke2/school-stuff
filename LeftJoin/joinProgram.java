// this program simulates an sql left join using two formatted text files.
// no libraries or any short cuts were allowed.
...
public class joinProgram
{
    public static void main(String args[])
    {
        if(args.length==3)
        {
            try
            {
                //test augments: {"file1.txt","file2.txt","A,B"}
            ...
            if(!error)
            {
                ...
                {//for each tuple of numbers in table 1
                    ...
                    {//for each value of a join attribute in that row
                       //  System.out.print(table1[headInd1[j]].get(i)+" ");
                ...
                printTable(result,headInd2,table1.length);
            }
            else
    ...
    private static void printTable(ArrayList[] table,int[] skipRows, int skipOffset)
    {
        for (int i = 0; i < table[0].size(); i++)
        {//for a row in the table (tuple)
            for (int j = 0; j < table.length; j++) 
            {
                //for a coloumn in a table
                boolean skip = false;
                if(skipRows!=null)
                {//skip specified coloumns (duplicate coloumns)
                     for(int k=0;k<skipRows.length;k++)
                        if(skipRows[k]+skipOffset==j)
                            skip=true;
                }
